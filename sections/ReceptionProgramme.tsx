...
import { receptionProgramme } from "@/lib/data";
import { Section } from "@/components/ui/Section";
import { SectionHeading } from "@/components/ui/SectionHeading";
import { Reveal, StaggerGroup } from "@/components/ui/Reveal";

export function ReceptionProgramme() {
  return (
    <Section id="programme" className="bg-[color:var(--surface)]/40">
      <SectionHeading
        eyebrow="Let's Celebrate"
        title="Reception Programme"
        description="Here's how our reception will unfold — join the fun!"
        className="mb-14"
      />

      <StaggerGroup className="mx-auto flex max-w-xl flex-col gap-4">
        {receptionProgramme.map((item) => (
          <Reveal key={item.order}>
            <div className="flex items-center gap-4 rounded-2xl border border-[color:var(--border-soft)] bg-[color:var(--surface)] px-5 py-4">
              ...
              <PartyPopper size={16} className="ml-auto shrink-0 text-[color:var(--gold)]" />
            </div>
          </Reveal>
        ))}
      </StaggerGroup>
